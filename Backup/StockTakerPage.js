...
export class StockTakerPage {
  ...
  async navigateToModule(managerTag) {
    ...
    try {
      ...
      // Skips seamlessly if authorization token cache remains active
    }
  ...
  async verifyProductVisibleInGrid(productNameText) {
    console.log(`📌 Step 6a: Verifying entries loaded safely onto scanning grid layout context...`);
    // Robust selector handling dynamic textual data string variations safely
    const gridItem = this.page.locator('div.stockchecks-container, table, tr, td')
      .getByText(productNameText, { exact: false })
      .or(this.page.locator('tr:has-text("Amiibo"), tr:has-text("Dimensions"), tr:has-text("Splatoon")'))
      .or(this.page.locator('div.stockchecks-container table tbody tr').locator('visible=true'))
      .last();
    
    await expect(gridItem).toBeVisible({ timeout: 15000 });
  }
